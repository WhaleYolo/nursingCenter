    ...
    <div style="box-shadow: 0 2px 4px rgba(0, 0, 0, .12), 0 0 6px rgba(0, 0, 0, .04)">
      <div>
        <el-input prefix-icon="el-icon-search" type="text" style="width:200px" v-model="findName"
          oninput="if(value.length > 11)value = value.slice(0, 11)" placeholder="输入员工姓名..." clearable></el-input>
        <el-button style="margin:0 30px" type="success" @click="findByName">查询</el-button>
        <!-- 弹出框 -->
        <el-button type="primary" @click="dialogVisible = true;form.staff_type = '医生'" v-if="$store.getters.getAdmin">
          新增员工信息</el-button>
      ...
      <!-- 员工信息表 -->
      <el-table :data="staffData" stripe style="width: 100%">
...
export default {
  ...
  methods: {
    findByName() {
      ...
      this.$axios({
        method: 'get',
        url: 'http://localhost:8080/staff/findDoctor?name=' + this.findName,
    ...
    getList() {
      this.$axios({
        method: 'get',
        url: 'http://localhost:8080/staff/doctor',
        headers: { "Authorization": sessionStorage.getItem("token") }
      }).then((res) => {
        //console.log(res.data)
        this.staffData = res.data.data
